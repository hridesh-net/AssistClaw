// Package core defines AssistClaw's contract layer: the interfaces and pure
// data types that every subsystem implements. It imports only the standard
// library — no other internal package, and no third-party dependency. This is
// the dependency-inversion seam: implementation packages (providers, channels,
// tools, memory, ...) depend on core; core depends on nothing internal. Only
// the composition root (internal/kernel) is allowed to import everything.
//
// The provider contracts below were moved verbatim from internal/provider so
// that the model-agnostic types live at the contract layer. internal/provider
// re-exports them as type aliases for backward compatibility during the revamp.
package core
