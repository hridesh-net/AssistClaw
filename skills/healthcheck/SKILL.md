---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AssistClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AssistClaw cron scheduling for periodic checks, or version status checks on a machine running AssistClaw (laptop, workstation, Pi, VPS).
---

# AssistClaw Host Hardening

## Overview

Assess and harden the host running AssistClaw, then align it to a user-defined risk tolerance without breaking access. Use AssistClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AssistClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. AssistClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AssistClaw can access sensitive data.
...
### 2) Run AssistClaw security audits (read-only)

As part of the default read-only checks, run `assistclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `assistclaw security audit` (faster, non-probing)
2. `assistclaw security audit --json` (structured output)

Offer to apply AssistClaw safe defaults (numbered):

1. `assistclaw security audit --fix`

Be explicit that `--fix` only tightens AssistClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AssistClaw version/update status (read-only)

As part of the default read-only checks, run `assistclaw update status`.
...
- Credential hygiene notes (location of AssistClaw creds, prefer disk encryption)
...
- AssistClaw security audit (re-run)
...
After AssistClaw install or first hardening pass, run at least one baseline audit and version check:

- `assistclaw security audit`
- `assistclaw security audit --deep`
- `assistclaw update status`

Ongoing monitoring is recommended. Use the AssistClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `assistclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `assistclaw update status`
...
Before creating, `assistclaw cron list` and match on exact `name`. If found, `assistclaw cron edit <id> ...`.
If not found, `assistclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `assistclaw update status` (preferred for source checkouts and channels)
2. `npm view assistclaw version` (published npm version)

## AssistClaw command accuracy

Use only supported commands and flags:

- `assistclaw security audit [--deep] [--fix] [--json]`
- `assistclaw status` / `assistclaw status --deep`
- `assistclaw health --json`
- `assistclaw update status`
- `assistclaw cron add|list|runs|run`

Do not invent CLI flags or imply AssistClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AssistClaw compaction:
